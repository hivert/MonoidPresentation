...
database. The database consists of proofs in an ad-hoc format as compiled from
runs of the St Andrews oracles. This database is used to generate ROCQ proofs
certifying the decidability of the word problem using the accompanying tools as
described below.
...
is a tool for converting the St Andrews oracle produced proof format to a ROCQ
proof. The `check_proofs.py` script is a tool for running python "proof
checkers" on the St Andrews oracle format directly.
...
`requirements.txt` may be necessary
(e.g. by running `python3 -m pip install -r requirements.txt`). The
`check_proofs.py` script is mainly intended as a timing benchmark.
